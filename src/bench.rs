...
fn mul_asm(x: &W6x64, y: &W6x64, expected: &W6x64) {
    ...
    let mut result = W6x64::default();
    for _i in 0..1_000 {
        unsafe { mont_mul_384_asm(&mut result.v[0], &xx.v[0], &yy.v[0]); }
        yy = xx;
        xx = result;
    }
    assert_eq!(&result, expected);
...
// Harness for multiplication in assembly with inputs and expected result
pub fn bench_mul_asm(c: &mut Criterion) {
    c.bench_function("Multiplication in assembly X 1000 iterations", |b| {
